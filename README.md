...
| `download`   | `tools/download/` | `{ "code": …, "filename": … }` | Downloads a file through the [Browserless](https://www.browserless.io/) `/download` API (`BROWSERLESS_HOST`/`BROWSERLESS_TOKEN`): runs a Puppeteer function you supply that triggers a browser download, then writes the returned bytes to `/sandbox/<filename>` and returns the saved path. `filename` must be a plain name (no `/` or `..`). Use it to fetch binary files (PDFs, images, …) so other tools can read them from the sandbox. |
